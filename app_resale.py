...
if selected == "About Project":
    ...
    st.subheader(":blue[Technologies :]")
    st.write("""
    ...
    st.write("""
    -  This project aims to construct a machine learning model and implement it as a user-friendly online application in order to provide accurate predictions about the 
    resale values of apartments in Singapore.
    - This prediction model will be based on past transactions involving resale flats, and its goal is to aid both future buyers and sellers 
    in evaluating the worth of a flat after it has been previously resold.
    - Resale prices are influenced by a wide variety of criteria, including location, the kind of apartment, the total square footage, and the length of the lease.
    - The provision of customers with an expected resale price based on these criteria is one of the ways in which a predictive model may assist in the overcoming of these obstacles.
    """)
...
if selected == "Predictions":
    ...
    st.markdown("### :orange[Predicting Resale Price (Regression Task) (R2 score: 0.85)]")
    ...
    with st.form("form1"):
        ...
        if submit_button:
            # Validating storey_range input after submit button is clicked
            ...
            # Proceed only if storey_median is calculated
            if storey_median is not None:
                try:
                    # Load all necessary files
                    with open("D:/vs code/flat/scaler.pkl", 'rb') as file:
                        scaler_loaded = pickle.load(file)
                    with open("D:/vs code/flat/xgb_model.pkl", 'rb') as file:
                    ...
                    new_sample = np.array([[cbd_dist, np.log(min_dist_mrt), floor_area_sqm, lease_remain_years, np.log(storey_median)]])

                    # Apply the scaler
                    new_sample_scaled = scaler_loaded.transform(new_sample)

                   # Convert the scaled input to DMatrix format
                    new_sample_dmatrix = xgb.DMatrix(new_sample_scaled)

                   # Make the prediction
                    new_pred = loaded_model.predict(new_sample_dmatrix)[0]
